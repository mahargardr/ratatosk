                ...
                for index,row in df_group.iterrows():
                    ...
                    for sub_id in sub_ids:
                        ...
                        if mo.lower()+'id' in cell_cols:
                            ...
                            df_sheet.loc[(pd.isnull(df_sheet[col_name]) & (df_sheet['duplex_type']!=mo.lower()) & (pd.notnull(df_sheet['duplex_type']))) ,col_name] = "-"
...
class Auditor:
        ...
        if (target == '*'):
            ...
            if (type(target)==str):
                ...
                if ('[' in target):
                    ...
                    df_config.loc[df_config[param].isin(list_target), param+'_check'] = 'OK'
                    df_config.loc[(pd.notnull(df_config[param])) & (pd.isnull(df_config[param+'_check'])), param+'_check'] = 'NOK'
                    return df_config
                if ('(' in target):
                    target = target.strip("()")
            ...
            if (exact_target):
                try:
                   df_config.loc[pd.isnull(df_config[param]), param+'_check'] = 'NA'
                   df_config.loc[(pd.isnull(df_config[param+'_check'])) & (df_config[param] != df_config[param+'_ref']), param+'_check'] = 'NOK' 
                   df_config.loc[(pd.isnull(df_config[param+'_check'])) & (df_config[param] == df_config[param+'_ref']), param+'_check'] = 'OK'
                except:
                   print('df_config length : %s'%len(df_config))
            else:
                df_config[param+'_eq'] = self.convert_cond_value(df_config[[param]],target)
    ...
    def convert_cond_value(self,df, target):
        operator = re.search("(\W+)", target).group()
        operand = re.search("(\w+)", target).group()
